...
@export var pace: float = 1.0  # Factor for how hard they are to catch (affects speed and wait time)
...
func _physics_process(delta: float) -> void:
	...
	if agent.is_navigation_finished():
		is_waiting = true
		wait_time = randf_range(min_wait_time / pace, max_wait_time / pace)
		velocity = Vector3.ZERO
	else:
		velocity = direction * speed * pace
